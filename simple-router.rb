...
class SimpleRouter < Controller
  ...
  def switch_ready(datapath_id)
    ...
    send_flow_mod_add(
      datapath_id, priority:1,
      :match => Match.new(eth_type:0X0800,ipv4_src:IPAddr.new("192.168.5.5"),ipv4_dst:IPAddr.new("192.168.3.3")),
      instructions:instruction=create_instruction_from(Mac.new("00:00:00:00:01:00"),Mac.new("74:2b:62:7e:a5:7f"),1)
  )
    send_flow_mod_add(
      datapath_id, priority:2,
      :match => Match.new(eth_type:0X0800,ipv4_src:IPAddr.new("192.168.3.3"),ipv4_dst:IPAddr.new("192.168.5.5")),
      instructions:instruction=create_instruction_from(Mac.new("00:00:00:00:02:00"),Mac.new("74:2b:62:7d:66:94"),2)
   )

  end

  def packet_in(dpid, message)
    puts message.in_port
  ...
  def forward(dpid, message)
    ...
    if arp_entry
      ...
      puts message.in_port
      puts "eth_src"
      puts message.eth_src
      puts "eth_dst"
      puts message.eth_dst
      puts ""

      instruction = create_instruction_from(eth_src, eth_dst, interface.port)
      flow_mod dpid, message, instruction

      #instruction = create_instruction_from(message.eth_dst, message.eth_src, message.in_port)
      #flow_mod dpid, message, instruction
  ...
  def set_table_miss_flow_entry(dpid)
    ...
    send_flow_mod_add(
      ...
      instructions: [inst]
        ...
        #:dl_src => message.eth_dst,
        #:dl_dst => message.eth_src
